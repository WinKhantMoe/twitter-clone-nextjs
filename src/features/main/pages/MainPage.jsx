...
export default function MainPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("/Bottom_nav.json").then((res) =>
          res.json()
        );
    ...
    <div className="bg-black h-full">
      <div className="grid grid-cols-2 pt-20 ">
        ...
        <div>
          ...
          <h3 className="text-white text-3xl font-semibold mt-14">
            Join today.
          </h3>
          <div className="mt-8 ">
            <GoogleLogin
              onSuccess={(response) => {googleLoginUser(jwtDecode(response.credential)) 
              }}
          ...
          <div className="mt-4">
            <button onClick={()=>setCanUse(false)} className="bg-white  rounded-3xl hover:cursor-pointer text-black py-2 px-16 w-1/2 flex gap-2">
              <Image
                src="/apple_logo.png"
                width={20}
                height={20}
              ...
              <span className={`${canUse ? "translate-x-0" : "-translate-x-100"} text-nowrap transition-all ease-in-out duration-300 `}>Sign up with Apple</span>
              <span className={`${canUse ? "translate-x-100" : "-translate-x-30"} text-nowrap transition-all ease-in-out duration-300`}>Not supported</span>
              </div>
            </button>
          </div>
          ...
          <Dialog open={signUpOpen} onOpenChange={setSignUpOpen}>
            <DialogTrigger className="w-1/2 hover:cursor-pointer">
              <div className="mt-4">
          ...
          <div className="mt-10 text-lg text-white">
            ...
            <div className="mt-4">
              ...
              <a target="_blank" href="https://grok.com" >
                <button className="border hover:cursor-pointer border-gray-500  text-sm rounded-3xl text-white py-2 px-16 w-1/2">
      ...
      <div className="flex flex-wrap text-nowrap mt-10 gap-y-2 justify-center">
        {bottomNav?.map((nav, index) => (
          <span
            key={index}
            className={`border-0 border-gray-400 ${
              index === bottomNav.length - 1 ? "border-r-0" : "border-r-2"
            } px-5 text-xs text-gray-500 w-fit`}
          >
            {nav.name}
          </span>
        ))}
