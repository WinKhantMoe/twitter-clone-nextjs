...
export const ChooseDraftFile = ({ setDraftOpen }) => {
  ...
  const draftMediaInput = document.getElementById("draftMediaInput");
  const allData = {
    media : media,
    mediaURL : mediaURL,
    tweetText  : tweetText,
      ...
      <div className="flex justify-between">
        <div className="flex items-center text-blue-500 gap-3">
          ...
          <div className="relative group">
            ...
            <div className="absolute py-1 px-2 rounded-xs text-white bg-zinc-600 text-xs -left-1 top-8 group-hover:opacity-100 opacity-0">
              Poll
            </div>
          </div>
          <div className="relative group">
          ...
          <div className="relative group">
            ...
            <div className="bg-blue-500 absolute cursor-pointer size-10 rounded-full -left-2   -top-2.5 z-10 group-hover:opacity-10 opacity-0"></div>
            <div className="absolute p-1 rounded-xs text-white bg-zinc-600 text-xs -left-4 top-8 group-hover:opacity-100 opacity-0">
              Schedule
            </div>
          </div>
        ...
        <div className="flex gap-3 items-center">
          <div
            className={`${
              tweetText?.length > 0 || (tweetText === "" && "hidden")
          ...
          <button
            onClick={() => handleCreateDraftTweet(allData)}
            className={`bg-white cursor-pointer ${
              tweetText?.length > 0 ||
              tweetText === "" ||
              (media?.length > 0 && "opacity-50 pointer-events-none")
